...
export async function askClaudeWithRetry(prompt, options = {}, retries = 2, delay = 1000) {
  const { askClaude } = await import('./ClaudeService.js'); // or wherever it's defined
  for (let attempt = 1; attempt <= retries + 1; attempt++) {
    try {
      const result = await askClaudeWithRetry(prompt, options);
      if (result?.output?.length > 50) return result;
      console.warn(`⚠️ Claude response too short. Attempt ${attempt}`);
    } catch (err) {
      console.warn(`❌ Claude call failed (Attempt ${attempt}):`, err);
    }
    if (attempt <= retries) {
